...
class LangfuseTracer(BaseTracer):
    ...
    def add_event(
        ...
        In Langfuse, events are added as nested spans or via score/observation APIs.
        For simplicity, we add them as child spans with short duration.
        ...
        try:
            langfuse_obj = span.vendor_obj

            # Create an event as a nested span
            event_params: dict[str, Any] = {
                "name": event_name,
                "metadata": attributes or {},
            }

            # Use event() method if available (for Trace objects)
            langfuse_obj.start_observation(**event_params)

            if self.debug:
                logger.debug(f"Added Langfuse event: {event_name}")

        except Exception as e:
            logger.warning(f"Failed to add Langfuse event '{event_name}': {e}")
    ...
    def update_generation(
        ...
        """Update an LLM generation span with model-specific information.

        This is a Langfuse-specific method for updating generation details
        like model name, token usage, and parameters.

        Args:
            span: The LLM span to update
            model: Model name/identifier
            usage: Token usage dict with input_tokens, output_tokens, total_tokens
            model_parameters: Model parameters like temperature, max_tokens, etc.
        """
        if not self.enabled or span.vendor_obj is None:
            return

        if span.type != SpanType.LLM:
            logger.warning("update_generation called on non-LLM span")
            return

        try:
            generation_obj = span.vendor_obj
            update_params: dict[str, Any] = {}
            ...
            if model_parameters:
                update_params["model_parameters"] = model_parameters

            if update_params:
                generation_obj.update(**update_params)

            if self.debug:
                logger.debug(f"Updated Langfuse generation: {span.name}")

        except Exception as e:
            logger.warning(f"Failed to update Langfuse generation '{span.name}': {e}")
